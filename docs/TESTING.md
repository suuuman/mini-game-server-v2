...
| 2 | `.\scripts\zone.ps1 -Clients 8 -Zones 4 -Chats 100` | 존 경계 브로드캐스트. ⭐ **`-Cxx` 를 붙이면 접속·Enter·입장·채팅·판정을 `client.exe zone` 이 한다**(T016 · ADR-030 결정 5 — 스폰·가짜 세션 예약·`Stop-Harness` 는 같은 래퍼가 하고, PS 갈래는 무변경). PS↔C++ 동치는 `client.ps1` S24/S25 가 회귀 안에서 본다. ⚠️ **PS 갈래의 `-Churn` 모드는 낡았다** — known 집합이 `{103,102}`(ADR-026 이전)라 존 이동마다 오는 정상 `kZoneMembersNtf(112)` 를 「깨진 스트림」으로 오판해 `판정 ✕` 를 낸다(T016 Step 0 실측 — `T016-step0-zone-churn10.txt`). `-Cxx` 는 112 를 known 에 넣어 ○ 다(**의도된 비동치** · ADR-030 A12 · S27). PS 갈래 수정은 다음 소정리 후보 |
...
| 8 | `.\scripts\churn.ps1 -Count 1000 -Framed` | 접속·종료 반복 · 누수. ⭐ **`-Cxx` 를 붙이면 접속/교환/종료 루프만 `client.exe churn` 이 돌고** before/after 통계·델타 판정·ASan 거부는 래퍼 그대로다(T016 · ADR-030 결정 4·5). 이 갈래엔 `-Report` 마다 찍던 중간 서버 통계 줄이 없다(클라는 서버 프로세스를 관측하지 않는다). 첫 회차 접속 실패는 PS 와 달리 exit 3 으로 승격된다(정상 경로 동치만 `client.ps1` S22/S23 이 본다) |
...
| 15 | `.\scripts\client.ps1` | **C++ 클라이언트(`client.exe`) 회귀 43판정**(T015 35 · ADR-029 — S0 · S1 · S1b · S2×4 · S3a~f · S6 · S7 · S8~S21 · S5×3 · S4a~c / **T016 +8** · ADR-030 — S22~S29) — `selftest` 순수 함수 **17항목**(S0 — T015 12 + T016 `bytes_equal_n0`·`zone_of_round_robin`·`expect_count_floor`·`tally_feed_split`·`remap_ready_closed_peer`) · `send` 이식(S1 순서 3000 · S1b **`send.ps1` 과 순서 판정 1종 동치** · S3b/S3e/S8/S9/S12/S16 판정 분기) · `flow`(S2 login/enter/echo/pong 정상 · S3a/S3c/S3d/S3f/S13/S14/S15/S17/S18/S19 부정 대조군) · usage/exit 2·3(S6/S7/S10/S11/S20/S21) · **A15 주기 ping**(S4a~c — `idle_timeout_sec=2` 마을 B 에서 ping 1s → 7s 생존 `pongs≥5` / ping 없음 → 절단 · `idle_kicked=1`) · 마을 A 종료 로그(S5 `[CONN ] rejected=0` · `[NET  ] idle_kicked=0` · `[ENTER]` 정확히 2) · **T016 — PS↔C++ 동치·실패 경로·usage 경계**: S22(`churn.ps1` PS 갈래 ○ · 마을 A 생존 중) · S23(`churn.ps1 -Cxx` exit 0 + `churn : count=300 failed=0` + 래퍼 `판정 ○`) · S26(`client churn --count 3 --msg-id 2 --framed` — 게이트 뒤 id 라 3/3 실패 → exit 1 · `failed=3` — 뮤턴트 MUT-C1·C2 판정자) · S28/S29(`client zone --clients 3 --zones 4` → exit 2 · `--clients 1025` → exit 2 — 서버 불요) · S24(`zone.ps1` PS 갈래 ○ · 전 서버 정지 뒤 스스로 스폰) · S25(`zone.ps1 -Cxx` exit 0 + `zone  : clients=8 zones=4 expect=200 joined=8 bad=0 broken=0 closed=0`) · S27(`zone.ps1 -Cxx -Churn 10` exit 0 · `broken=0 closed=0` · `other-ids` 0건 — A12 실증 · MUT-Z5 판정자). ⚠️ ASan 회차는 S22/S23 을 SKIP 으로 기록한다(`churn.ps1` 이 ASan 서버를 스스로 거부 — 43 그대로). **종료 코드 = 실패 건수**(0 이면 43/43 · Release 약 35~45s). ⛔ **단독 실행**(아래 절) · 서버 소스가 안 바뀐 클라 작업의 회귀는 이것 1종 + 자체 스폰 대표 1종(`zone.ps1`)으로 충분하다(서버 바이너리가 같은 소스에서 재빌드됐다는 전제 — T015 8단계 절차) |
...
<sup>i</sup> 클라만 바뀌고 서버 소스가 0줄이면 15 하나로 충분하다(S1b 가 `send.ps1` 동치를, **T016 부터는 S22/S24 가 `churn.ps1`·`zone.ps1` PS 갈래를** 하네스 안에서 함께 돌린다 — 자체 스폰 대표 1종은 S24 가 겸한다). **◐ 2 는 `.sln`·`.vcxproj` 가 바뀌어 서버 exe 가 재빌드됐을 때** — 자체 스폰 대표 1종으로 기동·회귀 무이상을 본다(T015 8단계 절차 · T016 은 `.sln` 무변경이라 S24 로 갈음). 클라 소스는 `proto/packet.h` 만 include 하므로 와이어 규약(`packet.h`)이 바뀌면 15 는 **프로토콜 행**에서 ● 로 걸린다.
...
| ~~⭐ **신설(T015) — 클라 `bytes_equal` 의 `n==0` 분기**(`memcmp` 회피 갈래)~~ | ✅ **닫혔다(T016)** — selftest `bytes_equal_n0`(빈 벡터·`nullptr`·n=0 → true / `{1}` vs `{2}` n=1 → false)가 그 분기를 직접 두드린다. 원문: 「미배선 — `selftest` 의 `bytes_equal` 항목은 8B 만 시험하고 `flow` 의 `--echo-size` 는 전부 기본값 8」(11단계 test 렌즈 LOW) | T016 이 selftest 를 12 → 17 로 늘리며 흡수(ADR-030 결정 8) |
| ⭐ **신설(T016) — 949 콘솔에서의 한글 판정 줄 매칭**(`client.ps1` S1 `order : OK — N 개가` · S22/S23/S24 `판정  : ○` · `-like` 한글 패턴 전부) | ⛔ **환경 의존 · 안 쟀다** — 이 저장소의 하네스 회차는 전부 콘솔 코드페이지 65001(Claude Code 세션 콘솔)에서 돌았고, 시스템 OEMCP/ACP 는 949 다(T016 lead 실측). PowerShell 5.1 은 자식 exe/스크립트 출력을 `[Console]::OutputEncoding` 으로 디코딩하므로 949 콘솔에서는 UTF-8 한글이 mojibake 가 돼 그 매칭이 조용히 실패할 수 있다 — 949 콘솔 회차를 돌리지 않았다 | T016 부터 **기계 계약은 ASCII**(종료 코드 · `RESULT:` · `zone  : …` / `churn : …` 요약 줄)로 두고 한글 줄은 사람용이다(ADR-030 결정 6). S22/S24 의 `판정 ○` 매칭은 PS 래퍼 자체의 판정이라 그대로 뒀다(T015 S1 과 같은 의존) — 949 콘솔에서 돌리려면 `chcp 65001` 을 먼저 |
| ⭐ **신설(T016) — `pump_once` 의 「닫힌 peer 가 섞인」 인덱스 리매핑** | ⛔ **창을 못 만든다** — 정상 서버 앞에서 일부 peer 만 도중에 닫히는 조합은 타이밍 의존이라 결정적으로 재현되지 않는다(S25·S27 전부 `closed=0`) | 리매핑을 순수 함수 `remap_ready` 로 빼서 selftest `remap_ready_closed_peer`(`open_index={0,2,3}·ready={1,2}→{2,3}`)가 직접 두드린다 — 7단계 test 렌즈 MED · 뮤턴트 MUT-Z6 의 판정자 |
| ⭐ **신설(T016) — `select_readable` 의 빈 집합 가드**(`valid_count==0 → select 미호출`) | ⛔ **창을 못 만든다** — peer 전원이 닫힌 뒤에도 `pump_once` 가 불리는 경로가 정상 시나리오에 없다(닫히면 판정으로 끝난다) | 가드의 근거는 MS Learn select 오류표(빈 fd_set → `WSAEINVAL`)와 코드 정독뿐 — 제거 뮤턴트는 생존이 자명해 주입하지 않았다(7단계 test 제안 · 기각 사유 기록) |
| ⭐ **신설(T016) — `client zone` 의 송신 실패 갈래**(`send_or_mark` — ②④ 에서 `send_all` 실패 → `closed`·`error`) | ⛔ **창을 못 만든다** — loopback 에서 Enter 뒤의 JoinZone/Chat 송신이 실패하려면 서버가 그 순간 끊어야 하는데 예약된 세션을 서버가 끊는 경로가 없다 | PS 는 `Stream.Write` 예외로 스크립트가 죽지만 C++ 은 그 peer 를 `closed` 로 표시하고 계속한다(의도된 비동치 · 7단계 correctness LOW) — 유지 근거는 코드 정독 |
| ⭐ **신설(T016) — `client churn` 의 「서버 없음」 갈래**(첫 회차 connect 실패 → exit 3) | ⚠️ **PS 와 비동치** — `churn.ps1` 은 첫 회차 실패도 `catch { $failed++ }` 로만 센다. Step 2 수동 실측(`--port 9019` → exit 3)만 있고 회귀에는 없다(사전 기동 서버가 있어야 도는 하네스라 「서버 없음」 을 회귀 안에서 만들 수 없다) | S22/S23 동치는 정상 경로 한정 — `cmd_churn.cpp` 머리말이 이 갈래를 명시한다 |
| ⭐ **신설(T016) — `-Cxx` 갈래의 `zone.ps1` 「서버 생존」 판정** | ⚠️ **의미가 다르다** — PS 는 `Get-Process village` 로 서버 프로세스 생존을 보지만 클라는 서버 프로세스를 관측하지 않는다 — `client zone` 은 「닫힌 소켓 0」(`closed=0`)으로 같은 질문에 답한다(문구에 `(closed=0)` 부기) | 서버가 죽으면 소켓이 전부 끊겨 `closed>0` 으로 드러난다 — 동치 근거는 코드 정독 · S27 이 정상 경로만 본다 |
