...
`kSerialDrainBatch`(16)는 공정성 상한이다. 한 세션이 직렬 큐에 수십 개를 들고 있어도(예: 순서 하네스가 짧은 간격으로 프레임을 밀어 넣는 경우) 워커 하나를 계속 독점하지 못하게, 상한만큼 비우고 잔량이 있으면 실행권 홀드를 쥔 채로 큐 뒤에 다시 선다(`worker_pool.h:21-25`). 이 숫자 자체는 안 쟀다 — 8단계에서 `send -Seq`·`zone_block` 지연 실측으로 재평가한다.
