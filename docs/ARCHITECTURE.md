...
| 단위 테스트 | **서버 쪽은 없음** — 클라 순수 함수만 `client selftest` **17항목**(`src/client/frame_codec.h` 의 조립·분할·str16·순번 검사·판정 술어 12 — T015 · ADR-029 결정 6 / `src/client/socket_set.h` 의 `zone_of`·`expect_count`·`tally_feed`·`remap_ready` + `bytes_equal n==0` 5 — T016 · ADR-030 결정 8) | 서버 순수 함수(`frame_size`, `world::Trade` 순수 메서드, 락 순서 정렬)는 여전히 테스트 가능하나 미착수 |
...
- 회귀: `send.ps1` → `zone.ps1` → `members.ps1` → `inventory.ps1` → `zone_block.ps1`(워커-풀 격리로 재정의) → `trade.ps1` → `zone_race.ps1`(락 검사기 assert 관찰로 재목적) → `churn.ps1` → `s2s.ps1` → `session.ps1` → `client.ps1`(뒤 셋은 서버를 스스로 스폰하는 단독 실행 — `client.ps1` 은 `client.exe` **43판정** · A15 주기 ping 실측 · T016 부터 `zone.ps1`/`churn.ps1` 의 PS 갈래와 `-Cxx`(C++ 트래픽) 갈래를 안에서 함께 돌려 동치를 본다)
