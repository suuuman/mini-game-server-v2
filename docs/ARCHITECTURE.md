...
`kSerialDrainBatch`(16)는 공정성 상한이다. 한 세션이 직렬 큐에 수십 개를 들고 있어도(예: 순서 하네스가 짧은 간격으로 프레임을 밀어 넣는 경우) 워커 하나를 계속 독점하지 못하게, 상한만큼 비우고 잔량이 있으면 실행권 홀드를 쥔 채로 큐 뒤에 다시 선다(`worker_pool.h:21-25`). ⚠️ **이 숫자 자체는 아직 안 쟀다.** 한때 이 자리에 *"8단계에서 `send -Seq`·`zone_block` 지연 실측으로 재평가한다"* 고 적혀 있었는데, **그 단계는 지나갔고 재평가는 없었다** — 8단계가 실제로 잰 것은 직렬 큐의 **순서**(`send -Seq 3000` — ADR-025 U2)이지 이 상한이 만드는 **지연**이 아니다. ⇒ 재평가는 여전히 이월이고, 잴 때 지울 표시가 이 문장이다(P5).
